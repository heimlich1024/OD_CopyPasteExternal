...
class OD_OT_PasteFromExternal(bpy.types.Operator):
    """Object Cursor Array"""
    ...
    def execute(self, context):
        ...
        def OD_PasteFromExternal(_name, size):
            ...
            for x in mesh.uv_layers:
                mesh.uv_layers.remove(x)

            for uvMap in uvMaps:
                uv = mesh.uv_layers.new(name=uvMap[0][0])
                bm = bmesh.new()
                bm.from_mesh(mesh)
...
def menu_func(self, context):
    self.layout.operator(OD_OT_PasteFromExternal.bl_idname)

def register():
    bpy.utils.register_class(OD_OT_PasteFromExternal)
    bpy.types.VIEW3D_MT_object.append(menu_func)

def unregister():
    bpy.utils.unregister_class(OD_OT_PasteFromExternal)
